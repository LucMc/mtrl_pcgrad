...
class GradNorm(OffPolicyAlgorithm[GradNormConfig]):
    ...
    def _update_inner(self, data: ReplayBufferSamples, original_losses, task_weights) -> tuple[Self, LogDict]:
        ...
        def update_critic(
            ...
            ####
            _gn_state, task_weights, _original_losses = update_gn_weights(gn_state,
                                                                          original_losses,
                                                                          _critic,
                                                                          num_tasks,
                                                                          task_ids, 
            ...
            }, _gn_state, task_weights, _original_losses
        ...
        def update_gn_weights(gn_state, original_losses, critic, num_tasks, task_ids, next_q_value, alpha=0.1):
            ...
            # Get grads and loss for each task
            def get_task_loss(task_idx: int, task_weights: Array) -> Array:
                task_mask = task_ids[:, task_idx] == 1
                ...
                return task_loss(critic.params)
            ...
            def gn_loss(params):
                ...
                # Compute weighted losses and their gradients separately for each task
                weighted_losses = []
                task_grads = []
                for i in range(num_tasks):
                    loss_i = get_task_loss(i, task_weights)  # You'll need to implement this
                    weighted_loss = task_weights[i] * loss_i
                    grad_i = jax.grad(lambda p: get_task_loss(i, p))(task_weights)
                    weighted_losses.append(loss_i)
                    task_grads.append(grad_i)
                
                task_losses = jnp.stack(weighted_losses)
                task_grads = jnp.stack(task_grads)
                
                # Compute loss ratios using stop_gradient (equivalent to detach)
                _original_losses = jax.lax.select(jnp.all(jnp.isnan(original_losses)), 
                                                task_losses, 
                                                original_losses)
                loss_ratio = jax.lax.stop_gradient(task_losses) / (_original_losses + 1e-12)
                
                # Compute relative inverse training rate
                rt = loss_ratio / jnp.mean(loss_ratio)
                
                # Compute gradient norms
                grad_norms = jnp.linalg.norm(task_grads, axis=1, ord=2)
                avg_grad_norm = jax.lax.stop_gradient(jnp.mean(grad_norms))
                
                # Compute GradNorm loss
                constant = jax.lax.stop_gradient(avg_grad_norm * rt ** alpha)
            ...
            return _gn_state, task_weights, _original_losses
